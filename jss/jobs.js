...
function insertOrdersToDb(){
	...
	if(zip == "" || zip == null){
		...
		if(isNaN(zip)){
			invalidZip = true;
			msgError = msgError + '- Zip is invalid !\n';	
		}
	}

	if(credit == "" || credit == null){
		creditError = true;
		msgError = msgError + '- Credit Card Details is Empty !\n';
	}else{
		if(isNaN(credit)){
			creditError = true;
			msgError = msgError + '- Credit Card Number is invalid !\n';
		}
	}
	...
	if(month == "" || month == null){
		...
		if(isNaN(month)){
			monthError = true;
			msgError = msgError + '- Month is invalid !\n';
		}
	}
	...
	if(year == "" || year == null){
		...
		if(isNaN(year)){
			yearError = true;
			msgError = msgError + '- Year is invalid !\n';	
		}
	}

	if (nameError == true || invalidZip == true || emailError == true ||  addressError == true|| cityError == true|| stateError == true||  zipError == true||  creditError == true|| monthError == true || yearError== true){
		alert(msgError);
	}else{
		console.log("Before inert Php Call");
		var xmlhttp;
		...
		//window.location.href = "http://localhost/Kharche_javacoffee/jobs.php";
